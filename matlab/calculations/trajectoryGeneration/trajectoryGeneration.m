...
z_offset = 0.400;                           % [m]   offset from (0,0) in z direction
...
% angular velocity rev/s * 10 (gear ratio)
subplot(ik_rows, ik_columns, 5)
hold on
    plot(t_vect, q1_t/(2*pi)*10)
    plot(t_vect, q2_t/(2*pi)*10)
    %plot(t_vect, q1d,'b-')
    %plot(t_vect(1:end-1), q1diff,'rx')
hold off
grid on
title('q_1 vel')
xlabel('t [s]')
ylabel("q_1' [rev/s * 10 (gear ratio)]")
...
if sim
    ...
    for i=1:10:length(t_vect)
        ...
        hold on
            ...
            text(x_path_pos(i), z_path_pos(i)+0.1,['p_x = ',num2str(round(x_path_pos(i),1)), ', p_z = ', num2str(round(z_path_pos(i),1))],'Color','red','FontSize',14)
